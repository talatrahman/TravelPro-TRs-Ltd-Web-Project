    ...
    <div class="toolbar">
        ...
        <div class="dropdown-menu sort-menu sort-menu-mobile">
            ...
            <div class="sort-item st-icheck">
                <div class="st-icheck-item"><label> <?php echo __('New car', ST_TEXTDOMAIN); ?><input class="service_order" type="radio" name="service_order_m_<?php echo $format; ?>" data-value="new" checked/><span class="checkmark"></span></label></div>
            </div>
            <div class="sort-item st-icheck">
                <span class="title"><?php echo __('Price', ST_TEXTDOMAIN); ?></span>
                <div class="st-icheck-item"><label> <?php echo __('Low to High', ST_TEXTDOMAIN); ?><input class="service_order" type="radio" name="service_order_m_<?php echo $format; ?>"  data-value="price_asc"/><span class="checkmark"></span></label></div>
                <div class="st-icheck-item"><label> <?php echo __('High to Low', ST_TEXTDOMAIN); ?><input class="service_order" type="radio" name="service_order_m_<?php echo $format; ?>"  data-value="price_desc"/><span class="checkmark"></span></label></div>
            </div>
            <div class="sort-item st-icheck">
        ...
        <h3 class="search-string modern-result-string" id="modern-result-string"><?php echo balanceTags(STCars::get_instance()->get_result_string()); ?> <div id="btn-clear-filter" class="btn-clear-filter" style="display: none"><?php echo __('Clear filter', ST_TEXTDOMAIN); ?></div> </h3>
    </div>
<?php
    ...
    <div id="modern-search-result" class="modern-search-result">
        ...
        if($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                echo st()->load_template('layouts/modern/car/elements/loop/' . $style, '', array('top_search' => true));
            }
        }else{
    ...
    <div class="pagination moderm-pagination" id="moderm-pagination" data-layout="normal">
        ...
        <span class="count-string">
            ...
            if ($query->found_posts):
                ...
                echo sprintf(__('%d - %d of %d %s', ST_TEXTDOMAIN), $posts_per_page * ($page - 1) + 1, $last, $query->found_posts, ($query->found_posts == 1 ? 'Car' : 'Cars'));
            endif;
            ?>
        </span>
    </div>