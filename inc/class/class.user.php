...
if (!class_exists('STUser_f')) {
    class STUser_f extends TravelerObject
    {
                ...
                case 'general':
                    ...
                    if($valid){
                        ...
                        if (!empty($post_id)) {
                            $my_post = [
                                'ID' => $post_id,
                                'post_title' => esc_html(STInput::request('st_title')),
                                'post_content' => STInput::request('st_content'),
                                'post_excerpt' => esc_html(stripslashes(STInput::request('st_desc'))),
                            ];
                            ...
                            update_post_meta( $post_id, 'total_time[hour]', STInput::request( 'total_time[hour]' ) );
                            update_post_meta( $post_id, 'total_time[minute]', STInput::request( 'total_time[minute]' ) );
            ...
            switch ($step_name){
                case 'basic_info':
                    ...
                    if($valid){
                        ...
                        $arr_err = array('st_title', 'st_content', 'st_desc', 'rental_number', 'rental_max_adult', 'rental_max_children');
                ...
                case 'info':
                    ...
                    if(!empty($post_id)){
                        ...
                        if($valid){
                            ...
                            update_post_meta( $post_id, 'venue-facilities', STInput::request( 'venue-facilities' ) );
                            ...
                            $err_array = array('duration', 'venue-facilities');
            ...
            switch ($step){
                ...
                case 2:
                    ...
                    if(isset($_REQUEST['max_people']) && !empty($_REQUEST['max_people'])){
                        ...
                        $min_people = $_REQUEST['min_people'];
                        if($min_people > $max_people){
                            $validator->set_error_message('min_people', __("Number min people can not more than number max people", ST_TEXTDOMAIN));
                        }
                    }
                    break;
                case 3:
        ...
        private function stTourValidate($step = 1){
            ...
            switch ($step){
                case 1:
                    ...
                    $validator->set_rules('st_content', __("Content", ST_TEXTDOMAIN), 'required');
                    $validator->set_rules('st_desc', __("Short Intro", ST_TEXTDOMAIN), 'required');
                    $validator->set_rules('contact_email', __("Email", ST_TEXTDOMAIN), 'required|valid_email');
                ...
                case 'prices':
                    ...
                    if(!empty($post_id)) {
                        ...
                        if($valid) {
                            ...
                            update_post_meta($post_id, 'st_allow_cancel', STInput::request('st_allow_cancel'));
                            update_post_meta($post_id, 'st_cancel_number_days', (int)STInput::request('st_cancel_number_days'));
                            update_post_meta($post_id, 'st_cancel_percent', STInput::request('st_cancel_percent'));
        ...
        function validate_hotel_room()
        {
            ...
            if (STInput::request('st_allow_cancel') == 'on') {
                $validator->set_rules('st_cancel_percent', __("Cancellation percentage", ST_TEXTDOMAIN), 'required|unsigned_integer');
                $cancel_percent = STInput::request('st_cancel_percent');
                $partner_commission = st()->get_option('partner_commission', '0');
                if ($cancel_percent <= $partner_commission) {
                    $validator->set_error_message('st_cancel_percent', __("The cancellation percentage does not match the criteria.", ST_TEXTDOMAIN));
                }
            }
            ...
            if (wp_verify_nonce(STInput::request('st_update_post_cars', ''), 'user_setting')) {
                ...
                if (!empty($post_id)) {
                    ...
                    if (!empty($_REQUEST['journey_title'])) {
	                    ...
	                    update_post_meta($post_id, 'journey', array());
                    }
        ...
        function st_update_post_type_rental()
        {
            ...
            if (wp_verify_nonce(STInput::request('st_update_post_rental', ''), 'user_setting')) {
                ...
                if (!empty($post_id)) {
                    ...
                    self::$msg = [
                        'status' => 'success',
                        'msg' => __('Rental updated successfully!', ST_TEXTDOMAIN)
                    ];
                    ...
                    self::$msg = [
                        'status' => 'danger',
                        'msg' => __('Error : Unable to update rental!', ST_TEXTDOMAIN)
                    ];
                }
            }
        }
