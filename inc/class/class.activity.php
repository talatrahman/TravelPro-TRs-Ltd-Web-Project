        ...
        class STActivity extends TravelerObject
        {
            ...
            public function st_filter_activity_ajax(){
                ...
                if ( have_posts() ) {
	                ...
	                echo '<h3 class="ajax-filter-not-found">' . __( 'No activity found', ST_TEXTDOMAIN ) . '</h3>';
                }
            ...
            function get_result_string()
            {
                ...
                if ( $query->found_posts ) {
                    if ( $query->found_posts > 1 ) {
                        $p1 = esc_html( $query->found_posts ) . __( ' activities ', ST_TEXTDOMAIN );
                    } else {
                        $p1 = esc_html( $query->found_posts ) . __( ' activity ', ST_TEXTDOMAIN );
                    }
                } else {
                    $p1 = __( 'No activity found', ST_TEXTDOMAIN );
                }
            ...
            function do_add_to_cart()
            {
                ...
                if($min_number > 0){
                    if ( $adult_number + $child_number + $infant_number < $min_number ) {
                        STTemplate::set_message( sprintf( __( 'Min of people for this activity is %d people', ST_TEXTDOMAIN ), $min_number ), 'danger' );
                        $pass_validate = false;
