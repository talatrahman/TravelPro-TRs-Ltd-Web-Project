    ...
    class TravelHelper
    {
        ...
        public static function add_menu_below($nav_menu, $args)
        {
            ...
            if ($menu_style == '3' && !is_user_logged_in() && $args->theme_location == 'primary') {
                ...
                <a href="" class="login" data-toggle="modal"
                   data-target="#st-login-form">' . esc_html__('Login', ST_TEXTDOMAIN) . '</a>
            </li>' . '<li class="topbar-item signup-item">
                    <a href="" class="signup" data-toggle="modal"
                       data-target="#st-register-form">' . esc_html__('Sign Up', ST_TEXTDOMAIN) . '</a>
                </li>';
                $nav_menu .= '</ul>';
            }
        ...
        public static function pag_filter_author($query)
        {
            if (!is_admin() && $query->is_main_query()) {
                if ($query->is_author()) {
                    $query->set('post_type', array('st_hotel', 'st_activity', 'st_tours'));
                }
            }
        }
        ...
        */
            ...
            $defaults = [
                ...
                'logged_in_as' => '<p class="logged-in-as">' . sprintf(
                    /* translators: 1: edit user link, 2: accessibility text, 3: user name, 4: logout URL */
                        __('<a class="st-link" href="%1$s" aria-label="%2$s">Logged in as %3$s</a>. <a class="st-link" href="%4$s">Log out?</a>'),
                        get_edit_user_link(),
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <textarea name="comment" class="form-control has-matchHeight"
                                          placeholder="<?php echo __('Comment*', ST_TEXTDOMAIN) ?>"></textarea>
                            </div>
                            ...
                            <div class="col-xs-12 col-sm-4">
                                <input type="url" class="form-control" name="url"
                                       placeholder="<?php echo __('Webiste', ST_TEXTDOMAIN) ?>">
                            </div>
                        </div>
                        <?php
        ...
        static function pagination_comment($args = [])
        {
            $defaults = [
                'range' => 4,
                'total' => false,
                'previous_string' => __('Prev', ST_TEXTDOMAIN),
                'next_string' => __('Next', ST_TEXTDOMAIN),
                'before_output' => '<div class="pagination"><ul>',
                'after_output' => '</ul></div>'
            ];
            $args = wp_parse_args(
                $args,
                apply_filters('st_pagination_comment_defaults', $defaults)
            );
            ...
            if ($previous && (1 == $page)) {
                $echo .= '<li class="disabled"><a class="page-item" href="javascript:void(0);" title="' . esc_html__('previous', ST_TEXTDOMAIN) . '">' . balanceTags($args['previous_string']) . '</a></li>';
            }
            if ($previous && (1 != $page)) {
                $echo .= '<li class=""><a class="page-item" href="' . esc_url($previous) . '" title="' . esc_html__('previous', ST_TEXTDOMAIN) . '">' . balanceTags($args['previous_string']) . '</a></li>';
            }
            ...
            if ($next && ($count == $page)) {
                $echo .= '<li class="disabled"><a class="page-item" href="javascript:void(0);" title="' . esc_html__('next', ST_TEXTDOMAIN) . '">' . balanceTags($args['next_string']) . '</a></li>';
            }
            if ($next && ($count != $page)) {
                $echo .= '<li class=""><a class="page-item" href="' . esc_url($next) . '" title="' . esc_html__('next', ST_TEXTDOMAIN) . '">' . balanceTags($args['next_string']) . '</a></li>';
            }
            if (isset($echo))
