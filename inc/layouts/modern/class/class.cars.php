...
if (!class_exists('STCars')) {
    class STCars extends TravelerObject
    {
        ...
        public function st_filter_cars_ajax()
        {
            ...
            <span class="count-string">
                ...
                if ($wp_query->found_posts):
                    ...
                    echo sprintf(__('%d - %d of %d %s', ST_TEXTDOMAIN), $posts_per_page * ($page - 1) + 1, $last, $wp_query->found_posts, ($wp_query->found_posts == 1 ? 'Car' : 'Cars'));
                endif;
                ?>
            </span>
            <?php
        ...
        function do_add_to_cart()
        {
            ...
            $selected_destination = STCars::get_route($location_id_pick_up, $location_id_drop_off);
            $extras = STInput::request('extra_price', []);
            $extra_price = self::geExtraPrice($extras,$check_in_n,$check_out_n);

            $info_price = STCars::get_info_price($post_origin, strtotime($check_in), strtotime($check_out), $location_id_pick_up, $location_id_drop_off);
            $price_unit = $info_price['price'];
            $item_price = floatval(get_post_meta($post_origin, 'cars_price', true));
            if ($item_price < 0) $item_price = 0;

            $price_destination = STCars::get_route($location_id_pick_up, $location_id_drop_off);
        ...
        function geExtraPrice($extra_price = [],$check_in_n,$check_out_n)
        {
            $date1 = strtotime($check_in_n);  
            $date2 = strtotime($check_out_n); 
            ...
            if (isset($extra_price['value']) && is_array($extra_price['value']) && count($extra_price['value'])) {
                foreach ($extra_price['value'] as $name => $number) {
                    ...
                    $total_price += $price_item * $number_item *$days_extra;
                }
            }
