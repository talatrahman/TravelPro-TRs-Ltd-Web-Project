        ...
        class STTour extends TravelerObject
        {
            ...
            public function st_filter_tour_ajax_location(){
                ...
                <span class="count-string">
                    ...
                    if ($query_service->found_posts):
                        ...
                        echo sprintf(__('%d - %d of %d %s', ST_TEXTDOMAIN), (int)$posts_per_page * ($page - 1) + 1, $last, $query_service->found_posts, ($query_service->found_posts == 1 ? 'Hotel' : 'Hotels'));
                    endif;
                    ?>
                </span>
                <?php
                ...
                if ( get_post_type( $post_id ) == 'st_tours' ) {
                    ...
                    if ( !empty( $all_stats ) and is_array( $all_stats ) ) {
                        ...
                        foreach ( $all_stats as $key => $value ) {
                            if ( isset( $st_review_stats[ trim($value[ 'title' ]) ] ) ) {
                                $total_point += $st_review_stats[ trim($value[ 'title' ]) ];
                                //Now Update the Each Stat Value
                                update_comment_meta( $comment_id, 'st_stat_' . sanitize_title( $value[ 'title' ] ), $st_review_stats[ trim($value[ 'title' ]) ] );
                            }
                        }
            ...
            */
            ...
            public function checkHasTourPackage($hotel_package, $hotel_package_custom, $activity_package, $activity_package_custom, $car_package, $car_package_custom, $flight_package, $flight_package_custom){
               if(!empty($hotel_package) && (is_object($hotel_package) && !empty((array)$hotel_package))){
                   return true;
               }
                if(!empty($hotel_package_custom) && (is_object($hotel_package_custom) && !empty((array)$hotel_package_custom))){
                    return true;
                }
                if(!empty($activity_package) && (is_object($activity_package) && !empty((array)$activity_package))){
                    return true;
                }
                if(!empty($activity_package_custom) && (is_object($activity_package_custom) && !empty((array)$activity_package_custom))){
                    return true;
                }
                if(!empty($car_package) && (is_object($car_package) && !empty((array)$car_package))){
                    return true;
                }
                if(!empty($car_package_custom) && (is_object($car_package_custom) && !empty((array)$car_package_custom))){
                    return true;
                }
                if(!empty($flight_package) && (is_object($flight_package) && !empty((array)$flight_package))){
                    return true;
                }
                if(!empty($flight_package_custom) && (is_object($flight_package_custom) && !empty((array)$flight_package_custom))){
                    return true;
                }
