        ...
        class STPaymentGateways
        {
                ...
                if ( isset( $all[ $id ] ) ) {
                    ...
                    if ( method_exists( $value, 'get_name' ) ) {
                        ...
                        if($order_code || $order_token_code) {
                            ...
                            if ($status == 'incomplete') {
                                $result = false;
                                STTemplate::set_message(__("Your payment is incomplete.", ST_TEXTDOMAIN));
                            }
                        }else{
                            $result = false;
            ...
            static function package_do_checkout($gateway, $order_id){
                try {
                    ...
                    $res[ 'message' ]   = sprintf( __( 'Your order has been made but we can not process the payment with %s. %s', ST_TEXTDOMAIN ), $gateway->get_name(), $e->getMessage() );
                }
